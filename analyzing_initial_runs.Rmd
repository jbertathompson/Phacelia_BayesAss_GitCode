...
For wrapping up this project (running phacelia data through BayesAss), I'm returning to old runs and making sure we've mined them for anything useful. In exploring how BayesAss worked, I ran it 5 different ways on the phacelia dataset, as warm up. We learned some things about the dataset & program from this exercise, then moved on to more runs, improving methods based on what was learned from these initial tests and more reading. This script analyzes just the initial runs, and prepares plots for the report.

This version started from work in December, based on older work from June, then was copied to a github repository for improved version control moving forward (Jan 10). This repository is a dedicated code directory within the BayesAss folder. This requires recoding Rmd in order to explicitly point to where things should be gathered from and saved, instead of working within a working directory. Benefits - easy version control and less file spewing! I can keep working on a single script through time with the comfort of saving to the internet each day. 

input data and output for this script lives here:
...
***********************************************************
##Section 3. Gather data from 5 runs into dataframes, label, and summarize
In preparation for plotting, the raw data ready.
...
```{r}
load("Q:/Research/All_Projects_by_Species/Phacelia SPECIES/Phacelia_formosula/Phacelia formosula_abiotic/Modelling/BayesAss/Initial_exploration_completed_BayesAss_runs/fivenitial_runs_dataframe_12_18_17.Rdata")
```
...
#So far the dataframe contains the full MCMC search. Make a modified dataframe with just the posterior without the burnin.
```{r}
...
******************************************************************************************************************************************
## Section 4. Posterior distribution plots
Plot the posterior distributions for migration parameters for these 5 runs. 

#Ideas for things to add (as time)
...
Allows comparison between runs and evaluation of distribution. For this round, each auto-scaled to see shapes of distribution nicely. 
```{r}
#which columns contain migration rates? Not the first two (Iteration, LogProb), not the last one (Run).
...
nothing <- lapply(m_indices, function(m){
  ...
  #Save as pdf for quick reference (easy to open file)
  filename1 = paste("m_postdist", mig, thism, "5initialruns_autoplot.pdf", sep = "_")
  filepath1 = paste(path, "plots_posterior/", filename1, sep = "")
  ggsave(file = filepath1, width = 7, height = 4, units = "in")
  
  #save as .eps so that it can read into illustrator as vectors for figure making. Main reason necessary is that pdf, while vector, does a weird discretization of heatmap color bar in legend, while the eps ones come out smooth. The other difference is that filling in the distributions with an alpha doesn't happen in the .eps, and I kinda like it that way. on the fence about feature. 
  filename2 = paste("m_postdist", mig, thism, "5initialruns_autoplot.eps", sep = "_")
...
#Same as above, but with y axis fixed to 2,000,000, so all the plots have the same y axis for direct comparison of how pointy of squashed they are.
```{r}
#which columns contain migration rates? Not the first two (Iteration, LogProb), not the last one (Run).
m_indices <- c(1:length(names(allfivepostburn.df)))[-c(1:2,length(names(allfivepostburn.df)))]
map <- mappinglol[[1]]#take first mapping. They are all the same (otherwise this data organization wouldn't work!)
nothing <- lapply(m_indices, function(m){
  ...
  filename1 = paste("m_postdist", mig, thism, "5initialruns_yfixed.pdf", sep = "_")
  filepath1 = paste(path,"plots_posterior/", filename1, sep = "")
  ggsave(file = filepath1, width = 7, height = 4, units = "in")
  
  
  filename2 = paste("m_postdist", mig, thism, "5initialruns_yfixed.eps", sep = "_")
  filepath2 = paste(path,"plots_posterior/", filename2, sep = "")
  ggsave(file = filepath2, width = 7, height = 4, units = "in")
...
#Same as above but with x axis fixed to 0-1.0, so x and y fixed, and prior boundaries marked. 
```{r}
#which columns contain migration rates? Not the first two (Iteration, LogProb), not the last one (Run).
m_indices <- c(1:length(names(allfivepostburn.df)))[-c(1:2,length(names(allfivepostburn.df)))]
map <- mappinglol[[1]]#take first mapping. They are all the same (otherwise this data organization wouldn't work!)
nothing <- lapply(m_indices, function(m){
  ...
  filename1 = paste("m_postdist", mig, thism, "5initialruns_yfixedx1.pdf", sep = "_")
  filepath1 = paste(path,"plots_posterior/", filename1, sep = "")
  ggsave(file = filepath1, width = 7, height = 4, units = "in")
  
  
  filename2 = paste("m_postdist", mig, thism, "5initialruns_yfixedx1.eps", sep = "_")
...
nothing <- lapply(m_indices, function(m){
  ...
  filename1 = paste("m_postdist", mig, thism, "5initialruns_xfixedyauto.pdf", sep = "_")
  filepath1 = paste(path, "plots_posterior/", filename1, sep = "")
  ggsave(file = filepath1, width = 7, height = 4, units = "in")
  
  
  filename2 = paste("m_postdist", mig, thism, "5initialruns_xfixedyauto.eps", sep = "_")
  filepath2 = paste(path, "plots_posterior/", filename2, sep = "")
  ggsave(file = filepath2, width = 7, height = 4, units = "in")
...
#auto axis limits, like first, but without scale limits - more padding around numbers. 
```{r}
#which columns contain migration rates? Not the first two (Iteration, LogProb), not the last one (Run).
m_indices <- c(1:length(names(allfivepostburn.df)))[-c(1:2,length(names(allfivepostburn.df)))]
map <- mappinglol[[1]]#take first mapping. They are all the same (otherwise this data organization wouldn't work!)
nothing <- lapply(m_indices, function(m){
  ...
  filename1 = paste("m_postdist", mig, thism, "auto_5initialruns_autopad.pdf", sep = "_")
  filepath1 = paste(path,"plots_posterior/", filename1, sep = "")
  ggsave(file = filepath1, width = 7, height = 4, units = "in")
  
  filename2 = paste("m_postdist", mig, thism, "auto_5initialruns_autopad.eps", sep = "_")
  filepath2 = paste(path,"plots_posterior/", filename2, sep = "")
  ggsave(file = filepath2, width = 7, height = 4, units = "in")
...
#Plots refined to fit in report
All the above were useful exploration. Now, one more, with a few tweaks so that a few example posteriors can go in report. 
I think the example pairs I'll use are:
low migration: D->WA (and WA->WA) 
intermediate migration: AS->R (R->R)
high migration: WS->RN (RN->RN)
Showing the migration rate and the fraction of sink population derived from sink population. 

1) make them a little squatter so that 6 fit nicely on a page
2) y fixed the way I did it cuts off data (weird) - let y be free
3) 0 axis padding is weird - looks better with distribution flush against axis - but annoying on top of y that it hits - see if you can pad y auto.
4) x should be fixed to prior as appropriate

#fixed x to prior range, auto y with a little padding on top
```{r}
#which columns contain migration rates? Not the first two (Iteration, LogProb), not the last one (Run).
m_indices <- c(1:length(names(allfivepostburn.df)))[-c(1:2,length(names(allfivepostburn.df)))]
map <- mappinglol[[1]]#take first mapping. They are all the same (otherwise this data organization wouldn't work!)
...
******************************************************************************************************************************************
## Section 5. Migration rate matrix heat maps
